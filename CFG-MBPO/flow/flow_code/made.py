# from https://github.com/kamenbliznashki/normalizing_flows
...
class RealNVP(nn.Module):
    ...
    def log_prob(self, x, y=None):
        u, sum_log_abs_det_jacobians = self.forward(x, y)
        if torch.isnan(sum_log_abs_det_jacobians).any():
            print('tensor has nan value')
            sum_log_abs_det_jacobians[torch.isnan(sum_log_abs_det_jacobians)] = 0
        return torch.sum(self.base_dist.log_prob(u) + sum_log_abs_det_jacobians, dim=1)
...
def train(model, dataloader, optimizer, epoch):
    ...
    for i, data in enumerate(dataloader):
        ...
        x = x.view(x.shape[0], -1).to(torch.device("cpu"))
        ...
        if i % 20 == 0:
            print('epoch {:3d} / {}, step {:4d} / {}; loss {:.4f}'.format(
                epoch,50, i, len(dataloader), loss.item()))
