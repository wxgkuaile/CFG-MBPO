...
def readParser():
    ...
    # new add
    parser.add_argument('--MPCHorizon', type=int, default=6)
    ...
    parser.add_argument('--use_discriminator', default=True)
    ...
    parser.add_argument('--use_algo', type=str, default='discriminator',help='discriminator, flowrl, mbpo')
...
def train(args, env_sampler, predict_env, agent, env_pool, model_pool, couple_flow, rewarder, model_agent, rollout_schedule):
    ...
    #    actor_path = f"{args.checkpoint_path}_actor.pth"
    #    critic_path = f"{args.checkpoint_path}_critic.pth"
    ...
    for epoch_step in range(args.num_epoch):
        ...
        for i in count():
            ...
            if args.use_algo == 'discriminator':
                if args.env_name == 'AntTruncatedObsEnv-v2':
                    if epoch_step <= 150:
                        if i == 0:
                            train_predict_model(args, env_pool, predict_env, total_step, rewarder)
                        # function2
                ...
                if cur_step > 0 and cur_step % args.model_train_freq == 0 and args.real_ratio < 1.0:
                    ...
                    cumulative_error = rollout_model(args, predict_env, agent, model_pool, env_pool, rollout_length)
                    epoch_cumulative_error += cumulative_error  # Accumulate error for this epoch
            ...
            if total_step % args.epoch_length == 0:
                ...
                with open(file_name, "a") as file:
                    file.write(f"{total_step}\t{sum_reward}\t{epoch_cumulative_error}\n")
                    
                # Save the model checkpoint using SAC's own save_model method
                if total_step % 20000 == 0:
                  actor_path = f"{folder_path}/{args.env_name}_sac_actor_{total_step}.pth"
                  critic_path = f"{folder_path}/{args.env_name}_sac_critic_{total_step}.pth"
                  agent.save_model(args.env_name, suffix=str(total_step), actor_path=actor_path, critic_path=critic_path)

                logging.info(f"Step Reward: {total_step} {sum_reward} | Epoch Cumulative Error: {epoch_cumulative_error}")
...
def train_predict_model(args, env_pool, predict_env, total_step, rewarder):
...
def rollout_model(args, predict_env, agent, model_pool, env_pool, rollout_length):
    ...
    cumulative_error = 0
    rollout_error_file_path = f"./results/{args.env_name}_rollout_step_errors.txt"
    for i in range(rollout_length):
        # TODO: Get a batch of actions
        action = agent.select_action(state)
        next_states, rewards, terminals, info = predict_env.step(state, action)
        # If next_state is not available, you might need to get it from some other source
        true_next_states = next_state[:len(state)]  # Ensure the size match for the current batch
        print(f'next_states shape:{next_states.shape}, true_next_states shape:{true_next_states.shape}')
        error = F.mse_loss(torch.tensor(next_states, dtype=torch.float32), torch.tensor(true_next_states, dtype=torch.float32))
        cumulative_error += error.item()  # Accumulate the error
        ...
        with open(rollout_error_file_path, "a") as error_file:
            error_file.write(f"Rollout_length:{i+1}\t Cu_Error:{cumulative_error}\n")
        
    print(f"Cumulative Error for this rollout: {cumulative_error}")
    return cumulative_error

def MPC_rollout_model(args, predict_env, agent, model_pool, env_pool, rollout_length, rewarder, total_step):
    state, _, _, _, _ = env_pool.sample_all_batch(args.rollout_batch_size)  # (rollB, _dim) 100000
    st = time.time()
    for t in range(rollout_length):
        action = agent.select_action(state)
        next_states, rewards, terminals = predict_env.ParaMPCact(state, action, t, rollout_length, agent, rewarder)
        model_pool.push_batch([(state[j], action[j], rewards[j], next_states[j], terminals[j]) for j in range(state.shape[0])])
        nonterm_mask = ~terminals.squeeze(-1)
        if nonterm_mask.sum() == 0:
            break
